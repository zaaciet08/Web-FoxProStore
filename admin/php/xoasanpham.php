...
if (isset($_POST['pro_id'])) {
    $order_id_to_delete = $_POST['pro_id'];
    ...
    $query = "DELETE FROM tbl_products WHERE pro_id = ?";
    ...
    if ($affected_rows > 0) {
        ...
        echo json_encode(array('status' => 'success', 'message' => 'Sản phẩm đã được xóa khỏi danh sách.'));
    } else {
        // Không có dòng nào bị ảnh hưởng, có thể do không có thay đổi hoặc order_id không hợp lệ
        echo json_encode(array('status' => 'error', 'message' => 'Sản phẩm không được xóa.'));
    }
