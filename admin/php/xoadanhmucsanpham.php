...
if (isset($_POST['dm_id'])) {
    ...
    $query = "DELETE FROM tbl_danhmuc WHERE pro_danhmuc = ?";
    $stmt = mysqli_prepare($conn, $query);

    // Kiểm tra trạng thái prepared statement
    if ($stmt) {
    mysqli_stmt_bind_param($stmt, "i", $dm_id_to_delete);
    ...
    if ($affected_rows > 0) {
        ...
        echo json_encode(array('status' => 'success', 'message' => 'Danh mục sản phẩm đã được xóa khỏi danh sách.'));
    } else {
